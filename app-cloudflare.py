...
# -------------------------------------------------------------
# 1. Cross-Platform Cloudflare Tunnel Auto-Launcher
# -------------------------------------------------------------
@st.cache_resource
def run_tunnel_and_print():
    is_windows = platform.system() == "Windows"
    binary_name = "cloudflared.exe" if is_windows else "cloudflared"

    # Locate binary or auto-download locally
    cloudflared_path = shutil.which(binary_name) or shutil.which("cloudflared")
    if not cloudflared_path:
        local_bin = os.path.join(os.getcwd(), binary_name)
        if os.path.exists(local_bin):
            cloudflared_path = local_bin
        else:
            try:
                if is_windows:
                    sys.stdout.write("\nDownloading cloudflared.exe for Windows...\n")
                    sys.stdout.flush()
                    url = "https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-windows-amd64.exe"
                    urllib.request.urlretrieve(url, local_bin)
                    cloudflared_path = local_bin
                else:
                    sys.stdout.write("\nDownloading cloudflared for Linux...\n")
                    sys.stdout.flush()
                ...
                sys.stdout.write(f"\nFailed to auto-download cloudflared: {e}\n")
                sys.stdout.flush()
                return False

    def _worker():
        try:
            # Map upstream host header so WebSockets route through the tunnel properly
            cmd = [
                cloudflared_path, "tunnel",
                "--url", "http://127.0.0.1:8501",
                "--http-host-header", "localhost:8501"
            ]
            ...
            for line in iter(proc.stdout.readline, ''):
                ...
                if match:
                    url = match.group(0)
                    sys.stdout.write("\n" + "=" * 60 + "\n")
                    sys.stdout.write(f"🚀 PUBLIC CLOUDFLARE URL:\n👉 {url}\n")
                    sys.stdout.write("=" * 60 + "\n\n")
                    sys.stdout.flush()
                    break
        except Exception as err:
            sys.stdout.write(f"\nError running tunnel: {err}\n")
            sys.stdout.flush()
...
# -------------------------------------------------------------
# 5. Tolerant Retrieval Utilities
# -------------------------------------------------------------
